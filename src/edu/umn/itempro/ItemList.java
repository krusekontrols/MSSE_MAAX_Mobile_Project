...
public class ItemList extends ListActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		getContentResolver().registerContentObserver(ItemProProvider.getContentURI(ItemProDatabase.TABLE_CATEGORY), true,
		...
		{
			Intent intent = new Intent(ConfirmEdit.SET_EDIT_MODE);
	    	startActivityForResult(intent, REQUEST_SET_EDIT_MODE);
		}
	...
	public boolean dispatchKeyEvent(KeyEvent event){
		if (event.getAction() == KeyEvent.ACTION_DOWN)
			if (event.getKeyCode() == KeyEvent.KEYCODE_DPAD_LEFT){
				...
				if (pos > 0 && pos != Spinner.INVALID_POSITION){
					pos--;
				...
				int pos = spinner.getSelectedItemPosition();
				if (pos < spinner.getCount()-1 && pos != Spinner.INVALID_POSITION){
					pos++;
					spinner.setSelection(pos, true);
				}
				getListView().requestFocus();
	...
	private void updateCategories(long setID) {
		...
		int nameColumn = categoriesCursor.getColumnIndex(Category.NAME);
		int idColumn = categoriesCursor.getColumnIndex(Category._ID);
	...
	private Set<Long> fetchValidIDs() {
		...
		Cursor cursor = getContentResolver().query(ItemProProvider.getContentURI(ItemProDatabase.TABLE_ITEM), null, Item.ONLIST + " = 1", null, null);
	...
	private void handleCreateClick() {
		Intent intent = new Intent(Intent.ACTION_INSERT, ItemProProvider.getContentURI(ItemProDatabase.TABLE_ITEM));
	...
	private void updateList() {
		...
		if (text.equals(""))
			...
			selection = ItemProDatabase.TABLE_ITEM + "."
					+ Item.NAME + " LIKE '%" + text + "%'";
		...
		if (categoryID != ALL_CATEGORIES) {
			if (selection == null || selection.equals(""))
				...
				selection += " AND ";
			selection += Item.CATEGORY + " = " + categoryID;
		}
        ...
        Cursor cursor = managedQuery(ItemProProvider.getContentURI(ItemProDatabase.TABLE_ITEM), null, selection, null, null);

		CursorAdapter adapter = new ItemListCursorAdapter(this, cursor);
	...
	public boolean onContextItemSelected(MenuItem item) {
		...
		case CONTEXTMENU_ITEM_DELETE: {
            Uri itemUri = ContentUris.withAppendedId(ItemProProvider.getContentURI(ItemProDatabase.TABLE_ITEM), info.id);
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
        Uri itemUri = ContentUris.withAppendedId(ItemProProvider.getContentURI(ItemProDatabase.TABLE_ITEM), id);
        Intent intent;
		switch (mode) {
		case EDIT_MODE:
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent intent) {
		...
		else if(requestCode == REQUEST_SET_EDIT_MODE && resultCode == RESULT_OK){
			setMode(EDIT_MODE);
		}
	}
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		menu.add(0, MENU_ITEM_EDIT_MODE, 0, R.string.edit_mode_label).setIcon(android.R.drawable.ic_menu_edit);
		menu.add(0, MENU_ITEM_SHOPPING_MODE, 1, R.string.shopping_mode_label).setIcon(android.R.drawable.ic_menu_directions);
		menu.add(1, MENU_ITEM_MAINTENANCE, 2, R.string.maintenance_label).setIcon(android.R.drawable.ic_menu_preferences);
		menu.add(1, MENU_ITEM_ABOUT, 3, R.string.about_label).setIcon(android.R.drawable.ic_menu_info_details);
		return true;
	}
	...
	private void showAboutBox() {
        ...
        .setTitle(getResources().getString(R.string.app_name) + " " + getResources().getString(R.string.version))
        .setMessage("Created by Team MAAX")
